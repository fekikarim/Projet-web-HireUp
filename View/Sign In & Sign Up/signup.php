...
if (
    ...
    if (
        ...
        if ($userC->userNameExists($user->get_user_name())){
            ...
            header('Location: ../../View/Sign In & Sign Up/authentication-register.php?error_user_name=' . urlencode($error_user_name) . '&user_name=' . urlencode($user->get_user_name()) . '&email=' . urlencode($user->get_email()));
        ...
        if ($userC->emailExists($user->get_email())){
            ...
            header('Location: ../../View/Sign In & Sign Up/authentication-register.php?error_email=' . urlencode($error_email) . '&user_name=' . urlencode($user->get_user_name()) . '&email=' . urlencode($user->get_email()));
            exit(); // Make sure to stop further execution after redirection
        }

        $userC->addUser($user);
        $success_message = "Account created successfully!";
        header('Location: ../../View/Sign In & Sign Up/authentication-login.php?success_global=' . urlencode($success_message) . '&user_name_email=' . urlencode($user->get_user_name()));
        exit(); // Make sure to stop further execution after redirection
    } else {
        // returning an error
