...
session_start();
    ...
    if ($current_reset_code == $real_reset_code){
        ...
        header('Location: ../../View/Sign In & Sign Up/change-password.php');
        exit();
    }
    else{  
        unset($_SESSION['reset code']);
        $error_wrong_verification_code = "Incorrect verification code. Please try again.";
        header('Location: ../../View/Sign In & Sign Up/forgot-password.php?error_verif_code=' . urlencode($error_wrong_verification_code) . '&user_name_email=' . urlencode($user_name_email));
