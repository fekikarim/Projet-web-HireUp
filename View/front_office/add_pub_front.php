...
include '../../Controller/pub_con_fron.php';
...
if (
    ...
    if (
        ...
        if ($pubb->titreExists($pub->get_titre())){
            ...
            header('Location: about.php?error_titre=' . urlencode($error_titre) . '&titre=' . urlencode($pub->get_titre()));
            exit(); // Make sure to stop further execution after redirection
        }

        $pubb->addpub($pub);
        $success_message = "pub added successfully!";
        header('Location: about.php?success_global=' . urlencode($success_message) . '&titre=' . urlencode($pub->get_titre()));
        exit(); // Make sure to stop further execution after redirection
    } else {
        // returning an error
